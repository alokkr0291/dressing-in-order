...
# def init_net(net, init_type='normal', init_gain=0.02, device=None, do_init_weight=True):
#     """
#     Initialize a network: 1. register CPU/MPS device; 2. initialize the network weights.

#     Parameters:
#         net (network)      -- the network to be initialized
#         init_type (str)    -- the name of an initialization method: normal | xavier | kaiming | orthogonal
#         init_gain (float)  -- scaling factor for normal, xavier, and orthogonal.
#         device (torch.device) -- The device to use for running the model (default: None, selects CPU/MPS automatically).
...
#     net.to(device)  # Move network to the selected device
...
def init_net(net, init_type='normal', init_gain=0.02, device=None, do_init_weight=True):
    if device is None:
        ...
        if torch.backends.mps.is_available():
            ...
            device = torch.device("cuda:0")  # Use first CUDA device
        else:
            device = torch.device("cpu")  # Fallback to CPU
    elif isinstance(device, list):
        # If device is a list, pick the first device ID
        if torch.backends.mps.is_available():
    ...
    print(f"Initializing network on device: {device}")  # Optional: Debugging print statement
    return net
